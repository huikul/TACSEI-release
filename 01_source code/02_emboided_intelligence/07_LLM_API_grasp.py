...
def main():
    ...
    completion = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": "You are the controller of a soft gripper. I will give the hardware description and various prompt, and you are required to generate various signal to control robot."},
            {
                "role": "user",
                "content": "Are you ready? Which kinds of description do you need?"
            }
        ]
    )
    '''
    ...
    file_path = 'prompts/03_task_hold_obj.txt'
    ...
    # sig_th, sig_ff, sig_mf = extract_arrays_with_find(completion.choices[0].message.content)
    # exec(completion.choices[0].message.content)
    '''
    # Use regex to extract the Python script between triple backticks
    import re
    match = re.search(r"```(.*?)```", completion.choices[0].message.content, re.DOTALL)
    if match:
        script = match.group(1).strip()  # Extract and strip any extra whitespace
    ...
    dir_new = dir_save_data + "/07_LLM_API_time{}".format(str(str_time))
    ...
    file_name = dir_new + "/03_task_hold_obj.txt"
    with open(file_name, "w", encoding="utf-8") as file:
        file.write(doc_task)
    #
    plt.plot(sig_th, label='sig_th')
    plt.plot(sig_ff, label='sig_ff')
    plt.plot(sig_mf, label='sig_mf')
    plt.title('')
    ...
    plt.savefig(file_name, format="png", dpi=200)  # dpi=300 for high resolution
    plt.show()
