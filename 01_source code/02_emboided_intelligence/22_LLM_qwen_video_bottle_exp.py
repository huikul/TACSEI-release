...
def worker(model, processor, prompt, dir_video):
    ...
    messages = [
        ...
        },
        {
            ...
            "content": [
                ...
                {
                    "type": "text",
                    "text": "Find the bottle and bottle cap, and describe them."
                },
            ],
        }
    ]
    ...
    '''
    frames, channels, h, w = video_inputs[0].shape
    if frames > 13:
        video_inputs[0] = video_inputs[0][0:13, :, :, :]
    '''
    inputs = processor(
        text=[text],
        images=image_inputs,
    ...
    messages = [
        ...
        },
        {
            ...
            "content": [
                ...
                {
                    "type": "text",
                    "text": "Considering the previous definition and the status of bottle cap. Tell me if the person is **opening or closing** the bottle cap, and explain the reason."
                },
            ],
        }
    ]
...
def main():
    ...
    dir_demostrations = home_dir + '/dataSSD/tamser_video/resize/560'
    lst_videos = get_file_name(dir_demostrations)

    file_path = 'prompts/11_knowledge_open_close.txt'
    doc_open_close = load_text_file(file_path)
    print(doc_open_close)
    #
    ...
    dir_new = dir_save_data + "/22_LLM_qwen_bottle"
    ...
    for cnt_video, dir_video in enumerate(lst_videos):
        ...
        if name_video.find('open') > -1 or name_video.find('close') > -1:
            print("############", name_video)
            result = worker(model=model, processor=processor, prompt=doc_open_close, dir_video=dir_video)
