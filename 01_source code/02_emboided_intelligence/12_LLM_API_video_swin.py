...
def main():
    dir_video = home_dir + '/dataSSD/tamser_video/swing_bottle_clockwise.mp4'
    ...
    print(doc_hardware)
    file_path = 'prompts/08_task_video_swing.txt'
    ...
    PROMPT_MESSAGES = [
        ...
        {   "role": "user",
            "content": [
                "These are frames from a video that I want to upload. From the camera's perspective: __Clockwise motion is defined as swinging from the top toward the right side__. __Counterclockwise motion is defined as swinging from the top toward the left side__. Please determine if __the object on the human hand swings clockwise or counterclockwise, referencing the features on upper body of the object__. ",
                *map(lambda x: {"image": x, "resize": 1080}, base64Frames[0:80:10]),
            ],
        },
    ]
    ...
    dir_new = dir_save_data + "/12_LLM_API_time{}".format(str(str_time))
    ...
    file_name = dir_new + "/08_task_video_swing.txt"
    with open(file_name, "w", encoding="utf-8") as file:
        file.write(doc_task)
