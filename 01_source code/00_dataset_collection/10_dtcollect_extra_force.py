...
def main():
    ...
    fildir_yaml = home_dir +"/dataSSD/TAMSER/TAMSER_python/00_dataset_collection/cfg/10_dataset_collection_extra_force.yaml"
    ...
    try:
        max_iter_air_change = int((cfg_info['dataset_collection']['max_air'] - cfg_info['dataset_collection']['min_air'])/cfg_info['dataset_collection']['step_air']) + 1
        for cnt_air_change in range(0, max_iter_air_change):
            # Sending a message
            # str_time = datetime.datetime.now().strftime('%Y%m%d%H%M%S%f')
            lst_info_save = []

            air_th_curr = int(cfg_info['dataset_collection']['min_air'] + cnt_air_change * cfg_info['dataset_collection']['step_air'])
